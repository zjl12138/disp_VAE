...
def main():
    ...
    cfg = dict(device=device, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay,
               last_observe=30, epochs=25, print_every=2, save_every=2, batch_size=5,
    ...
    kitti_dst = KittiDataset()
    train_loader = DataLoader(dataset=kitti_dst, batch_size=cfg['batch_size'], shuffle=True, num_workers=0, drop_last=True)

    model = torchvision.models.resnet50(pretrained=True, num_classes=1000) # TODO
    model.to(device)
    if RUN_PARALLEL:
    ...
    do_train(model, cfg, train_loader, optimizer, scheduler=None)


def do_train(model, cfg, train_loader, optimizer, scheduler):
    device = cfg['device']
    print_every = cfg['print_every']
    for e in range(cfg['epochs']):
        for i, (img_batch, map_batch) in enumerate(train_loader):
            model.train()
            # TODO
            img_batch = img_batch.to(device=device, dtype=torch.float)  # move to device, e.g. GPU
            target = model(img_batch)

            criterion = nn.MSELoss(reduction='mean')
            loss = criterion(img_batch, target)
